...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		var self = this,
			...
			TURK = "#00CAC6",
			special_char = "^",
			CO = { fs: 22, widt: 160, widt2: 230, fs2: 18, xx: 12, yy: 12, fs_sub: 14, fs_trans: 21, widt_trans: 230, subspace: 5 },
			LAB = { siz: 70, xx: W - MRG, yy: MRG },
			CTA = { fs: 12, hei: 32, toY: 206, xx: 12, yy: 13, rd: 6 },
			USP = { fs: 11, marg: 12, hei: 32, widt: 150, cy: 12, ty: 10 },
			POLL = { fs: 11, hei: 30, fon: ft1_reg, xx: 10, yy: 12, space: 6 },
			KEUZE = { fs: 18, hei: 48, fon: ft1, xx: 50, icon: 24, yy: 15, space: 12, fs_sub: 15, subw: 240, subspace: 0, backsize: 36, backrou: 6, bax: MRG * 2, bay: MRG * 2 },
			COUNT = { fs_big: 40, fs: 16, fs_small: 10 },
			leng = 1.5,
			raboH = 10 + 3,
			raboW = 110,
		...
		function fra(id, d) {
			var m = new cjs.MovieClip(),
				...
				keuze2 = false, // self.shottype == "keuze",
			...
			if (counter) {
				...
				for (var i = 0; i < cop.length; i++) {
					...
					copy(c, ftc, fsc, null, copies, col, toW, copy_delay + 0.3 + delc * 0.1, CO.xx, CO.yy + ch + xtr_y);
					if (typ == "big") {
						var num = getStr(c, "[", "]");
				...
				cop = cop[0];
				copy(cop, ft1, fs, 4, copies, col, toW, copy_delay + 0.3, trans ? -0 : CO.xx, cty);
			}
			if (trans) {
				var fh = copies.hei;
				if (sub) {
					copy(sub, ft1_reg, CO.fs_sub, null, copies, col, copy_w, copy_delay + 0.3 + copies.numChildren * 0.1, trans ? -0 : CO.xx, CO.yy + copies.hei + CO.subspace);
				...
				if (keuze2 && sub) {
					copy(sub, ft1, KEUZE.fs_sub, 4, copies, col, KEUZE.subw, copy_delay + 0.3 + copies.numChildren * 0.1, CO.xx, KEUZE.yy + bgrHei + KEUZE.subspace, 7);
					extr += copies.hei + 10;
					if (isRabo) {
						rabo = new lib.rabo();
				...
				if (!intr && !isUsp && !counter) copies.y = mr;
			}
			img.x = widt / 2;
			msk.x = widt;
			img.mask = msk;
			...
			bl.graphics.beginLinearGradientFill(["rgba(0, 51, 78, 1)","rgba(0, 51, 78, 0)"], [1, 0], 0, 0, 0, 60).drawRect(0, 0, W, 60);
			bl.y = H - 60;
			bl.alpha = 0.8;
		...
		function usp(m, d, id) {
			...
			for (var i = 0; i < usps.length; i++) {
				...
				con.y = Math.round(((hei + 6) - con.hei) / 2);
	...
	this.logo.name = "logo";
	this.logo.setTransform(233.05,327.5,0.7168,0.7153,0,0,0,76.8,14.8);
...
p.nominalBounds = new cjs.Rectangle(-150,275,438,63);
// library properties:
lib.properties = {
