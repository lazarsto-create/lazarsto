...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		var self = this,
			...
			TURK = "#00CAC6",
			special_char = "^",
			CO = { fs: 14, widt: 140, xx: 8, yy: 9 },
			LAB = { siz: 50, xx: W - 25 - MRG, yy: MRG + 25 },
			CTA = { fs: 9, hei: 22, toY: H - 22 - 6, xx: 10, yy: 10, rd: 6 },
		...
		function fra(id, d) {
			...
			if (trans) {
				...
				copies.y = mr // Math.round((H - bgrHei) / 2) // CTA.toY - mr - bgrHei;
				if (intr) copies.y = mr;
			}
			img.x = widt / 2;
			msk.x = widt;
			...
			bl.graphics.beginLinearGradientFill(["rgba(0, 51, 78, 1)","rgba(0, 51, 78, 0)"], [1, 0], 0, 0, 0, 40).drawRect(0, 0, W, 40);
			bl.y = H - 40;
			bl.alpha = 0.8;
	...
	this.logo.name = "logo";
	this.logo.setTransform(268.05,180.7,0.5213,0.5203,0,0,0,76.8,14.8);
...
p.nominalBounds = new cjs.Rectangle(-140,200,448,-10.199999999999989);
// library properties:
lib.properties = {
