...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		var self = this,
			...
			TURK = "#00CAC6",
			special_char = "^",
			CO = { fs: 24, widt: 195, widt2: 265, fs2: 21, fs3: 28, xx: 12, yy: 12 },
			LAB = { siz: 70, xx: W - MRG, yy: MRG },
			CTA = { fs: 12, hei: 32, toY: 236, xx: 12, yy: 13, rd: 6 },
			USP = { fs: 11, marg: 12, hei: 32, widt: 150, cy: 12, ty: 10 },
			COUNT = { fs_big: 48, fs: 18, fs_small: 10 },
			leng = 1.5,
			raboH = 10 + 3,
			raboW = 110,
		...
		function fra(id, d) {
			...
			if (counter) {
				...
				cop = cop[0];
				copy(cop, ft1, fs, 4, copies, col, toW, copy_delay + 0.3, trans ? -1 : CO.xx, CO.yy);
			}
			if (trans) {
				copies.y = 20 - CO.yy;
			...
			bl.graphics.beginLinearGradientFill(["rgba(0, 51, 78, 1)","rgba(0, 51, 78, 0)"], [1, 0], 0, 0, 0, 70).drawRect(0, 0, W, 70);
			bl.y = H - 70;
			bl.alpha = 0.8;
	...
	this.logo.name = "logo";
	this.logo.setTransform(266.05,356.9,0.7559,0.7542,0,0,0,76.8,14.8);
...
p.nominalBounds = new cjs.Rectangle(-132,290,456,78);
// library properties:
lib.properties = {
