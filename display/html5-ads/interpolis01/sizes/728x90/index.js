...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		var self = this,
			...
			TURK = "#00CAC6",
			special_char = "^",
			CO = { fs: 18, widt: 225, xx: 12, yy: 13 },
			LAB = { siz: 68, xx: 305, yy: H / 2 },
			CTA = { fs: 12, hei: 30, toY: MRG * 2, xx: 15, yy: 13, rd: 6 },
			USP = { fs: 11, marg: 18, hei: 38, widt: 190, cy: 15, ty: 10 },
			COUNT = { fs_big: 66, fs_small: 9, widt: 250 },
			leng = 1.5,
			raboH = 10 + 6,
			raboW = 130,
		...
		function f(c, fs, ft, toW, ml) {
			...
			if (tmp) { 
				st = " " + special_char + tmp;
			} else if (sp) st = " " + special_char + st.replace(" ", "");
		...
		function copy(co, fon, fs, ls, m, col, toW, d, toX, toY) {
			co = rohero(co);
			if (toW == undefined) toW = CO.widt;
			var c = f(co, fs, fon, toW, 3),
				nfs = c.fs,
			...
			for (i = 0; i < c.lines.length; i++) {
				...
				if (sp) {
					...
					for (var j = 0; j < txt.length; j++) {
						...
						var tf = new cjs.Text(tx, fss + "px '" + ft + "'", col);
						...
						if (HEY) {
							var num = getStr(messs, "[", "]");
							if (num.length > 0) num = Number(num);
							//tx = tx.replace("[", "");
							//tx = tx.replace("]", "");
							self.numb = { nu: 0, msg: messs, field: tf };
							self.numb.field.text = tx;
							//self.numb.field.updateCache();
							tl.to(self.numb, {duration: 2.5, nu: num, ease: "expo", onUpdateParams: [], onUpdate: countUp}, d);
						}
					}
				} else {
		...
		function countUp() {
			...
			if (c != self.numb.current) {
				...
				self.numb.current = c;
				if (self.numb.field.bitmapCache) {
					self.numb.field.updateCache();
				} else if (self.numb.field.parent.bitmapCache) {
					self.numb.field.parent.updateCache()
				}
			}
		}
		function fra(id, d) {
			...
			if (counter) copy_w = COUNT.widt;
			if (counter && cop[0].indexOf("big_") > -1) {
				var ch = 0,
					cw = 0,
					delc = 0,
					cou = 0;
				for (var i = 0; i < cop.length; i++) {
					...
					if (c.indexOf("_") > -1) {
						if (c.toLowerCase().indexOf("big_") > -1) {
							...
							if (i == 0) {
								xtr_y = -20;
								copies.bigID = copies.numChildren;
								var tst = new cjs.MovieClip(),
									tc = c.replace("[", "");
								tc = tc.replace("]", "");
								tc = tc.substring(tc.indexOf("_") + 1, tc.length);
								copy(tc, ftc, fsc, null, tst, col, copy_w, "none", 0, 0);
								var bigW = tst.children[0].w;
							}
					...
					copy(c, ftc, fsc, null, copies, col, copy_w, copy_delay + 0.3 + delc * 0.1, toX, CO.yy + ch + xtr_y);
					if (typ == "big") {
						var num = getStr(c, "[", "]");
						if (num.length > 0) num = Number(num);
						//c = c.replace("[", "");
				...
				if (counter) {
					for (var l = 0; l < cop.length; l++) {
						if (cop[l].indexOf("big_") > -1) {
							cop[l] = cop[l].replace("big_", "^");
						} else if (cop[l].indexOf("small_") > -1) {
							cop[l] = small = cop[l].replace("small_", "");
							cop.splice(l, 1);
						}
					}
					cop = cop.join(" ");
				...
				copy(cop, ft1, fs, 3, copies, col, copy_w, copy_delay + 0.3, trans ? -1 : CO.xx, CO.yy);
				if (typeof small !== "undefined") {
					var sm = new cjs.MovieClip();
					copies.addChild(sm);
					copy(small, ft1_reg, COUNT.fs_small, 3, sm, col, copy_w, copy_delay + copies.numChildren * 0.1 + 0.1, CO.xx, copies.hei + 18);
					copies.hei += 8;
				}
			}
			if (trans) {
				...
				if (copies.hei > 0) {
					bgrHei = copies.hei + CO.yy * 2 - 4;
					if (isUsp && isRabo) {
						bgrHei += raboH;
						copies.widt = Math.max(raboW, copies.widt);
					}
					if (bigW) {
						bgrHei = Math.max(bgrHei, 66);
					}
					if (bigW) copies.widt += bigW + 10;
					if (intr) bgrHei = Math.min(bgrHei, 78);
					drawShape(bgr, copies.widt + CO.xx * 2, bgrHei, true, RD, bgrColor, copy_delay);
				}
				copies.y = Math.round((H - bgrHei) / 2) // CTA.toY - mr - bgrHei;
				if (intr) copies.y = mr;
			}
			img.x = widt / 2;
			msk.x = widt;
			...
			if (intr) {
				...
				if (isUsp) {
					...
					d3 += 0.5;
					copies.hei = bgrHei;
					if (usps.indexOf("no_") > -1) usp(copies, d3, id);
				}
				// USP END
			}
			// gradient behind logo begin
			var bl = new cjs.Shape();
			bl.graphics.beginLinearGradientFill(["rgba(0, 51, 78, 1)","rgba(0, 51, 78, 0)"], [1, 0], 0, 0, 150, 0).drawRect(0, 0, 150, H);
			bl.x = W - 150;
			bl.alpha = 0.8;
			...
			// STICKER BEGIN
			if (isImg(lab)) {
				var cow = copies.widt + CO.xx * 2 + 49;
				if (S[(id - 1)] && getImg(lab).src == getImg("image" + (labid - 2)).src) {
					tl.to(self.pri, {duration: leng + 0.1, x: copies.x + cow, ease: "expo.inOut"}, d);
				} else {
			...
			if (id < S.length - 1) {
				d += LE;
				if (!intr) {
					tl.to(msk, {duration: leng * 0.6, scaleY: (H - mr * 2) / hei, y: mr * 1, x: W - mr, ease: "none"}, d - leng * 0.25);
					tl.to(msk, {duration: leng * 0.75, y: -25, scaleX: (W + 0) / (W - mr * 2), scaleY: (H + 50) / hei, ease: eaz2, overwrite: "auto"}, d + leng * 0.6);
	...
	this.logo.name = "logo";
	this.logo.setTransform(652,165.75,0.8342,0.8339,0,0,0,76.7,14.8);
...
p.nominalBounds = new cjs.Rectangle(64,195,652,-5.199999999999989);
// library properties:
lib.properties = {
