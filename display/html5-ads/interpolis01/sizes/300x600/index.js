...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		var self = this,
			...
			TURK = "#00CAC6",
			special_char = "^",
			CO = { fs: 26, widt: 225, xx: 17, yy: 19, fs_sub: 19, fs_trans: 32, widt_trans: 255, subspace: 9 },
			LAB = { siz: 104, xx: W - MRG * 2, yy: MRG * 2 },
			CTA = { fs: 16, hei: 38, toY: 486, xx: 15, yy: 13, rd: 6 },
			USP = { fs: 11, marg: 18, hei: 38, widt: 190, cy: 15, ty: 10 },
			POLL = { fs: 12, hei: 34, fon: ft1_reg, xx: 10, yy: 14, space: 6 },
			KEUZE = { fs: 18, hei: 48, fon: ft1, xx: 50, icon: 24, yy: 16, space: 12, fs_sub: 15, subw: 240, subspace: 0, backsize: 36, backrou: 6, bax: MRG * 2, bay: MRG * 2 },
			COUNT = { fs_big: 72, fs_small: 10 },
			leng = 1.5,
			raboH = 14 + 8,
			raboW = 140,
		...
		function doClick(e) {
			...
			var url = grid8.adColumns["Exit Url"];
			window.open(url);
		}
		...
		function copy(co, fon, fs, ls, m, col, toW, d, toX, toY, ml) {
			...
			for (i = 0; i < c.lines.length; i++) {
				...
				t.x = toX;
				t.cache(cx, -nfs - 5, w + 10, nfs + 18, ca);
				m.addChild(t);
			...
			if (counter) {
				...
				for (var i = 0; i < cop.length; i++) {
					...
					if (typ == "big") {
						...
						self.numb.field.text = c;
						self.numb.field.updateCache();
						tl.to(self.numb, {duration: 2.5, nu: num, ease: "expo", onUpdateParams: [], onUpdate: countUp}, copy_delay + delc * 0.1 + 0.2);
					}
				...
				cop = cop[0];
				copy(cop, ft1, fs, null, copies, col, copy_w, copy_delay + 0.3, trans ? -1 : CO.xx, cty);
			}
			if (trans) {
				var fh = copies.hei;
				if (sub) {
				...
				if (keuze2 && sub) {
					copy(sub, ft1, KEUZE.fs_sub, 4, copies, col, KEUZE.subw, copy_delay + 0.3 + copies.numChildren * 0.1, CO.xx, KEUZE.yy + bgrHei + KEUZE.subspace, 6);
					extr += copies.hei + 10;
					if (isRabo) {
						rabo = new lib.rabo();
						copies.addChild(rabo);
						rabo.x = copies.children[copies.numChildren - 2].x + 1;
				...
				copies.y = CTA.toY - mr - bgrHei - extr;
			}
			img.x = widt / 2;
			msk.x = widt;
			img.mask = msk;
		...
		function an() {
			...
			for (var i = 0; i < self.logo.fishes.numChildren; i++) {
				var f = self.logo.fishes.children[i],
					f2 = self.logo2.fishes.children[i],
					toX = f.x,
					toY = f.y,
					del = d + i * 0.04;
				...
				tl.to(f2, {duration: 0.2, motionPath: {path: [{x: toX - 40, y: toY}, {x: toX - 20, y: toY + 2}, {x: toX, y: toY}], autoRotate: true, curviness: 2.0}}, 0);
			}
	...
	this.logo.name = "logo";
	this.logo.setTransform(122.55,646.5);
...
p.nominalBounds = new cjs.Rectangle(-150,450,426,226);
// library properties:
lib.properties = {
