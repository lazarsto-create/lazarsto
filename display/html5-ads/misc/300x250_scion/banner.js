...
bannerboy.main = function() {
	...
	var width = 300;
	var height = 250;
	var banner = bannerboy.createElement({width: width, height: height, backgroundColor: "#333e48", overflow: "hidden", cursor: "pointer", parent: document.body});
	var mouseX = 0;
	var mouseY = 0;
	var canvas = bannerboy.createElement({type: "canvas", width: width, height: height, parent: banner});
	...
	bannerboy.preloadImages(images, function() {
		...
		var bg = bannerboy.createElement({backgroundImage: "bg.jpg", retina: true, parent: banner});

		/////////////////
		// T U B E  M A N

		// tube torso divs		
		var tube_top = bannerboy.createElement({backgroundImage: "tube.png", retina: true});
		var tube_bottom = bannerboy.createElement({backgroundImage: "tube.png", retina: true});
		var tube_height = tube_top.get("height");
		var tube_width = tube_top.get("width");
		var tube_ball = bannerboy.createElement({backgroundImage: "tube.png", scale: 0.95, width: tube_width, height: tube_width, borderRadius: "100%"});
		...
		// hair
		var tube_hair = bannerboy.createElement();
		var tube_hair_inner = bannerboy.createElement({backgroundImage: ["hair_1.png", "hair_2.png", "hair_3.png"], retina: true, parent: tube_hair});
		tube_hair_inner.set({x: -tube_hair_inner.get("width")/2 + 3, y: -tube_hair_inner.get("height")/2})

		// tube arm divs
		var tube_arm_r_1 = bannerboy.createElement({backgroundImage: "arm_r_1.png", retina: true});
		var tube_arm_r_2 = bannerboy.createElement({backgroundImage: "arm_r_2.png", retina: true});
		
		var tube_arm_l_1 = bannerboy.createElement({backgroundImage: "arm_l_1.png", retina: true});
		var tube_arm_l_2 = bannerboy.createElement({backgroundImage: "arm_l_2.png", retina: true});
		var arm_position = (tube_height - 35);

		// append
		var tube = bannerboy.createElement({width: width, height: height, scaleX: 1, parent: banner});
		...
		TweenLite.ticker.addEventListener("tick", function() {
			...
			tube_anim.swag_a += Math.pow(bannerboy.utils.map(Math.sin(tube_anim.swag_a), -1, 1, 0.05, 0.2), 0.8);
			...
			a1 += bannerboy.utils.shortestArc(a1, a1_target);
			lowerLength = Math.min(bannerboy.utils.dist(tube_anim.x, tube_anim.y, base_p.x, base_p.y), tube_height);
			...
			// update torso divs
			tube_bottom.set({x: base_p.x - tube_width/2, bottom: height - base_p.y, rotation: bannerboy.utils.toDegrees(a1 + Math.PI / 2), transformOrigin: "50% 100%", height: lowerLength});
			tube_top.set({x: mid_p.x - tube_width/2, y: mid_p.y - upperLength, rotation: bannerboy.utils.toDegrees(a2 + Math.PI / 2), height: upperLength, transformOrigin: "50% 100%"});
			tube_ball.set({scale: 0.90, rotation: bannerboy.utils.toDegrees((a1 + a2)/2 + Math.PI / 2), x: mid_p.x - tube_width / 2 + 1, y: mid_p.y - tube_width/2, backgroundPosition: "-2px " + (lowerLength - tube_height) + "px"});

			// hair
			tube_hair.set({x: head_p.x + Math.cos(a2) * 5, y: head_p.y + Math.sin(a2) * 5})
			tube_hair.to(0.2, {rotation: bannerboy.utils.toDegrees(a2 + Math.PI/2)})
			...
			a3 += bannerboy.utils.shortestArc(a3, a3_target);
			...
			tube_arm_r_1.to(0.5, {rotation: bannerboy.utils.toDegrees(a3 + Math.PI/2) + 45, transformOrigin: "6px 6px"});

			var a4 = tube_arm_r_1.get("rotation");
			var a4_rad = bannerboy.utils.toRadians(a4);
			...
			tube_arm_l_1.to(0.5, {rotation: bannerboy.utils.toDegrees(a3 + Math.PI/2) - 27 + Math.cos(q*2) * 20, transformOrigin: "100% 30%"});
		...
		var bg2 = bannerboy.createElement({width: width, height: 80, top: base_p.y + 10, backgroundImage: "bg.jpg", parent: banner}); // cover tube man
		bg2.style.backgroundSize = "100%";
		bg2.style.backgroundPosition = "0 100%";
		var txt_3 = bannerboy.createElement({backgroundImage: "txt_3.png", left: 67, top: 20, retina: true, parent: banner});
		var txt_2 = bannerboy.createElement({backgroundImage: "txt_2.png", left: 21, top: 20, retina: true, parent: banner});
		var txt_1 = bannerboy.createElement({backgroundImage: "txt_1.png", left: 21, top: 20, retina: true, parent: banner});
		var logo_im = bannerboy.createElement({backgroundImage: "logo_im.png", left: 30, top: 215, retina: true, parent: banner});
		var logo_ia = bannerboy.createElement({backgroundImage: "logo_ia.png", left: 235, top: 215, retina: true, parent: banner});
		var logo_scion = bannerboy.createElement({backgroundImage: "logo_scion.png", left: 125, top: 205, retina: true, parent: banner});
		var car_1 = bannerboy.createElement({left: -1, top: 127, parent: banner});
			var car_1_base = bannerboy.createElement({backgroundImage: "car_1_base.png", top: 36, retina: true, parent: car_1});
			var car_1_top = bannerboy.createElement({backgroundImage: "car_1_top.png", transformOrigin: "10px 60px", left: 5, retina: true, parent: car_1});
		var car_2 = bannerboy.createElement({left: 139, top: 130, parent: banner});
			var car_2_base = bannerboy.createElement({backgroundImage: "car_2_base.png", top: 34, retina: true, parent: car_2});
			var car_2_top = bannerboy.createElement({backgroundImage: "car_2_top.png", transformOrigin: "140px 60px", left: 10, retina: true, parent: car_2});
		var cta = createButton(51, 55, 198, 45, "cta_txt.png");
		var cursor = bannerboy.createElement({backgroundImage: "cursor.png", left: 227, top: 75, retina: true, parent: banner});
		...
		function createButton(left, top, width, height, labelImage) {
			var button = bannerboy.createElement({left: left, top: top, parent: banner});
			var shadow = bannerboy.createElement({width: width, height: height, top: 4, backgroundColor: "#003369", borderRadius: "4px", parent: button});
			var base = bannerboy.createElement({width: width, height: height, backgroundColor: "#1d4e8f", borderRadius: "4px", parent: button});
			var label = bannerboy.createElement({backgroundImage: labelImage, retina: true, parent: base});
			label.center();
