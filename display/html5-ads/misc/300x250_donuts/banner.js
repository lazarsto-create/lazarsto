...
bannerboy.main = function() {
	...
	var width = 300;
	var height = 250;
	var banner = bannerboy.createElement({width: width, height: height, background: "#FFF", border: "1px solid #000", boxSizing: "border-box", overflow: "hidden", perspective: 500, cursor: "pointer", parent: document.body});
	var introFade = bannerboy.createElement({background: "#FFF", width: width, height: height, zIndex: 1000, parent: banner});
	var colors = {
		color1: "#f8deb8",
		color2: "#e49b6a",
		color3: "#ec5fcf",
	...
	var circlesContainer = bannerboy.createElement({width: 1, height: 1, zIndex: 1, parent: banner}); circlesContainer.center();
	var circles = [
		{
			centerPoint: bannerboy.createElement({width: 1, height: 1, left: 0, right: 0, top: 0, bottom: 0, margin: "auto", parent: circlesContainer}),
			numCircles: 20,
			radius: 110,
			slice: 0,
			rotation: 0,
	...
	bannerboy.preloadImages(images, function() {
		...
		var endFrameDotPoint = bannerboy.createElement({background: "red", width: 10, height: 10, x: -100, display: "none", y: 80, parent: banner}); endFrameDotPoint.center();
		var cta = bannerboy.createElement({backgroundImage: "cta.png", left: 21, top: 148, retina: true, parent: banner});
		var ctaShine = bannerboy.createElement({backgroundImage: "ctaShine.png", scale: 3, parent: cta});
		var txt8_1 = bannerboy.createElement({backgroundImage: "txt8.png", left: 19, top: 96, clip: "rect(0 1000px 11px 0)", retina: true, parent: banner});
		var txt8_2 = bannerboy.createElement({backgroundImage: "txt8.png", left: 19, top: 96, clip: "rect(11px 1000px 1000px 0)",  retina: true, parent: banner});
		var txt7 = bannerboy.createElement({backgroundImage: "txt7.png", left: 19, top: 94, retina: true, parent: banner});
		var txt6_2 = bannerboy.createElement({backgroundImage: "txt6_2.png", left: 79, top: 132, retina:true, parent: banner});
		var txt6_1 = bannerboy.createElement({backgroundImage: "txt6_1.png", left: 83, top: 92, retina:true, parent: banner});
		var txt5_2 = bannerboy.createElement({backgroundImage: "txt5_2.png", left: 89, top: 125, retina:true, parent: banner});
		var txt5_1 = bannerboy.createElement({backgroundImage: "txt5_1.png", left: 90, top: 85, retina:true, parent: banner});
		var txt4_2 = bannerboy.createElement({backgroundImage: "txt4_2.png", left: 86, top: 124, retina: true, parent: banner});
		var txt4_1 = bannerboy.createElement({backgroundImage: "txt4_1.png", left: 87, top: 85, retina: true, parent: banner});
		var txt3 = bannerboy.createElement({backgroundImage: "txt3.png", left: 60, top: 96, retina: true, parent: banner});
		var txt2 = bannerboy.createElement({backgroundImage: "txt2.png", left: 39, top: 23, retina: true, parent: banner});
		var txt1_2 = bannerboy.createElement({backgroundImage: "txt1.png", left: 57, top: 17, retina: true, clip: "rect(15px 1000px 50px 0)", parent: banner});
		var txt1_1 = bannerboy.createElement({backgroundImage: "txt1.png", left: 57, top: 17, retina: true, clip: "rect(0 1000px 15px 0)", parent: banner});
		var shadow = bannerboy.createElement({backgroundImage: "shadow.jpg", left: 122, top: 187, zIndex: 0, parent: banner});
		var keurig = bannerboy.createElement({backgroundImage: "keurig.png", left: 86, top: 73, zIndex: 10, parent: banner});
		var smokeCanvas = bannerboy.createElement({type: "canvas", width: 87, height: 74, left: 4, top: 36, opacity: 0, display: "none", parent: keurig});
		var lid = bannerboy.createElement({backgroundImage: "lid.png", left: 74, top: 61, zIndex: 20, parent: banner});
		var logo_big = bannerboy.createElement({backgroundImage: "logo_big.png", left: 20, top: 66, retina: true, parent: banner});
		var logo = bannerboy.createElement({backgroundImage: "logo.png", left: 209, top: 230, zIndex: 100, retina: true, parent: banner});
		var smallDot = bannerboy.createElement({background: colors.color2, width: 25, height: 25, left: 135, top: 142, borderRadius: "100%", parent: banner});

		var smoke = new bannerboy.Sequence(smokeCanvas, imageSequence, 12, true);
			...
			for (var i = 0; i < circles.length; i++) {
				for (var j = 0; j < circles[i].dots.length; j++) {
					...
					if (i == 1) {
						...
						var distance = bannerboy.utils.dist(dot.get("x"), dot.get("y"), targetX, targetY);
						if (distance < currentNearestDistance) {
							currentNearestDistance = distance;
							nearestDot = dot;
						}
						// console.log(bannerboy.utils.dist(dot.get("x"), dot.get("y"), targetX, targetY));
	...
	function updateDots (circleObject) {
		for (var i = 0; i < circleObject.dots.length; i++) {
			...
			circleObject.rotation += bannerboy.utils.toRadians(circleObject.rotationSpeed);
