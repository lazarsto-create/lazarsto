...
(async function loadFeedAndImages(){
  try{
    ...
    // images array from column 'image3' (URLs) and include price16, price4
    const images = feedArr.map(r=>({
      image3: (r['image3'] ? r['image3'].trim() : null),
      data8: (r['data8'] ? r['data8'].trim() : ''),
      price16: (r['price16'] ? r['price16'].trim() : ''),
      price4: (r['price4'] ? r['price4'].trim() : '')
    ...
    // build carousel inside .carousel container (180x180 slides with text)
    ...
    carousel.style.width = `${selectedItems.length * (180 + gap)}px`;
    ...
    if(selectedItems.length === 1){
      ...
      // also hide pagination if only 1 product
    ...
    function positionAllPrice4Below(){
      requestAnimationFrame(() => {
        for(let i = 0; i < carousel.children.length; i++){
          ...
          if(desc && price4){
            const descTop = -75; // description position Y (moved down 10px)
            const descHeight = desc.offsetHeight;
            // price4 position = description top + description height + 5px
            const price4Top = descTop + descHeight + 5;
    ...
    function slideTo(index, onDone){
      ...
      if(currentIndex === 0 && target === len - 1){
        ...
        gsap.to(carousel, {x:`+=${step}px`, duration:0.6, ease:'power1.inOut', onComplete:()=>{
      ...
      gsap.to(carousel,{x:`-${offset}px`,duration:0.6,ease:'power2.inOut', onComplete:()=>{
        if(typeof onDone === 'function') onDone();
        updateDescriptionAnimation();
        animating = false;
      }});
    ...
    const TRANS_DUR = 600; // ms transition duration (matches gsap durations)
    function stopAuto(){
      autoPlaying = false;
    ...
    startAuto();
    // (old timeline-based autoplay removed — using timer-based autoplay above)
  } catch(err) {
